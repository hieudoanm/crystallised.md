(globalThis.TURBOPACK || (globalThis.TURBOPACK = [])).push([
	...
	(e) => {
			...
			let n = 'string' == typeof e ? e : e.source,
				r = {
					replace: (e, t) => {
						...
						return ((i = i.replace(eu.caret, '$1')), (n = n.replace(e, i)), r);
					},
		...
		var ep = (() => {
			...
			eL = /[\s\p{P}\p{S}]/u,
			eI = /[^\s\p{P}\p{S}]/u,
			ez = ec(/^((?![*_])punctSpace)/, 'u')
				.replace(/punctSpace/g, eL)
				.getRegex(),
			e$ = /(?!~)[\p{P}\p{S}]/u,
			eD = ec(/link|precode-code|html/, 'g')
				.replace(
					'link',
					/\[(?:[^\[\]`]|(?<a>`+)[^`]+\k<a>(?!`))*?\]\((?:\\[\s\S]|[^\\\(\)]|\((?:\\[\s\S]|[^\\\(\)])*\))*\)/,
				)
				...
				.replace('html', /<(?! )[^<>]*?>/)
				.getRegex(),
			eO = /^(?:\*+(?:((?!\*)punct)|[^\s*]))|^_+(?:((?!_)punct)|([^\s_]))/,
			eM = ec(eO, 'u').replace(/punct/g, eC).getRegex(),
			eP = ec(eO, 'u').replace(/punct/g, e$).getRegex(),
			eB =
				'^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)punct(\\*+)(?=[\\s]|$)|notPunctSpace(\\*+)(?!\\*)(?=punctSpace|$)|(?!\\*)punctSpace(\\*+)(?=notPunctSpace)|[\\s](\\*+)(?!\\*)(?=punct)|(?!\\*)punct(\\*+)(?!\\*)(?=punct)|notPunctSpace(\\*+)(?=notPunctSpace)',
			eU = ec(eB, 'gu')
				.replace(/notPunctSpace/g, eI)
				.replace(/punctSpace/g, eL)
				.replace(/punct/g, eC)
				.getRegex(),
			eH = ec(eB, 'gu')
			...
			eQ = ec(/^!?\[(ref)\](?:\[\])?/)
				.replace('ref', ex)
				.getRegex(),
			eK = ec('reflink|nolink(?!\\()', 'g')
				.replace('reflink', eX)
				.replace('nolink', eQ)
				.getRegex(),
			eV = /[hH][tT][tT][pP][sS]?|[fF][tT][pP]/,
			eJ = {
				_backpedal: eo,
				anyPunctuation: eF,
				autolink: eq,
			...
			e3 = {
				...e1,
				br: ec(ev).replace('{2,}', '*').getRegex(),
				text: ec(e1.text)
					.replace('\\b_', '\\b_| {2,}\\n')
					.replace(/\{2,\}/g, '*')
					.getRegex(),
			},
			e2 = { normal: e_, gfm: eE, pedantic: eN },
			e9 = { normal: eJ, gfm: e1, breaks: e3, pedantic: e0 },
			e6 = {
				'&': '&amp;',
				'<': '&lt;',
				'>': '&gt;',
		...
		function e8(e, t) {
			if (t) {
				if (eu.escapeTest.test(e)) return e.replace(eu.escapeReplace, e4);
			} else if (eu.escapeTestNoEncode.test(e))
				return e.replace(eu.escapeReplaceNoEncode, e4);
			return e;
		}
		function e5(e) {
			try {
				e = encodeURI(e).replace(eu.percentDecode, '%');
			} catch {
				return null;
			}
			return e;
		...
		function e7(e, t) {
			...
			for (; r < n.length; r++) n[r] = n[r].trim().replace(eu.slashPipe, '|');
			return n;
		}
		function te(e, t, n) {
			let r = e.length;
...
`),
						...
						if (i.loose)
							for (let e of i.items)
								for (let t of ((e.loose = !0), e.tokens))
...
`
				...
				reflink(e, t) {
					...
					if (
						...
						return tt(n, e, n[0], this.lexer, this.rules);
					}
				}
				emStrong(e, t, n = '') {
			...
			tr = class e {
				...
				constructor(e) {
					((this.tokens = []),
						...
						(this.options = e || ea),
						(this.options.tokenizer = this.options.tokenizer || new tn()),
						(this.tokenizer = this.options.tokenizer),
						(this.tokenizer.options = this.options),
						(this.tokenizer.lexer = this),
						(this.inlineQueue = []),
						(this.state = { inLink: !1, inRawBlock: !1, top: !0 }));
					let t = { other: eu, block: e2.normal, inline: e9.normal };
					(this.options.pedantic
						? ((t.block = e2.pedantic), (t.inline = e9.pedantic))
						: this.options.gfm &&
							((t.block = e2.gfm),
							this.options.breaks
								? (t.inline = e9.breaks)
...
`,
				...
				blockTokens(e, t = [], n = !1) {
					for (
						this.options.pedantic &&
						(e = e.replace(eu.tabCharGlobal, '    ').replace(eu.spaceLine, ''));
						e;
...
` + r.text),
				...
				inlineTokens(e, t = []) {
					...
					if (this.tokens.links) {
						...
						if (e.length > 0)
							for (
								;
								null != (i = this.tokenizer.rules.inline.reflinkSearch.exec(r));
					...
					for (
						;
						null != (i = this.tokenizer.rules.inline.anyPunctuation.exec(r));
			...
			ti = class {
				...
				code({ text: e, lang: t, escaped: n }) {
					let r = (t || '').match(eu.notSpaceStart)?.[0],
						i =
							e.replace(eu.endingNewline, '') +
							`
`;
					return r
...
`
				...
				image({ href: e, title: t, text: n, tokens: r }) {
					...
					let i = e5(e);
					if (null === i) return e8(n);
					e = i;
					let l = `<img src="${e}" alt="${n}"`;
					return (t && (l += ` title="${e8(t)}"`), (l += '>'));
				}
				text(e) {
			...
			ts = class e {
				...
				constructor(e) {
					((this.options = e || ea),
						(this.options.renderer = this.options.renderer || new ti()),
						(this.renderer = this.options.renderer),
						(this.renderer.options = this.options),
						(this.renderer.parser = this),
						(this.textRenderer = new tl()));
				}
				static parse(t, n) {
				...
				parse(e) {
					...
					for (let n = 0; n < e.length; n++) {
						...
						if (this.options.extensions?.renderers?.[r.type]) {
							let e = this.options.extensions.renderers[r.type].call(
								{ parser: this },
								r,
							);
							if (
			...
			ta = class {
				...
				provideLexer() {
					return this.block ? tr.lex : tr.lexInline;
				}
				provideParser() {
			...
			to = new (class {
					...
					return (
						e.forEach((e) => {
							...
							if (
								...
								let t = this.defaults.renderer || new ti(this.defaults);
							...
							if (e.hooks) {
								let t = this.defaults.hooks || new ta();
								for (let n in e.hooks) {
									if (!(n in t)) throw Error(`hook '${n}' does not exist`);
									if (['options', 'block'].includes(n)) continue;
									let r = e.hooks[n],
										i = t[n];
									ta.passThroughHooks.has(n)
										? (t[n] = (e) => {
												if (
													this.defaults.async &&
													ta.passThroughHooksRespectAsync.has(n)
												)
													return (async () => {
				...
				parser(e, t) {
					return ts.parse(e, t ?? this.defaults);
				}
				parseMarkdown(e) {
					return (t, n) => {
						let r = { ...n },
						...
						try {
							...
							let n = (
								i.hooks ? i.hooks.provideLexer() : e ? tr.lex : tr.lexInline
							)(t, i);
...
Please report this to https://github.com/markedjs/marked.`),
		...
		((tc.options = tc.setOptions =
			function (e) {
				return (
					to.setOptions(e),
					(tc.defaults = to.defaults),
					(ea = tc.defaults),
					tc
				);
			}),
			...
			(tc.walkTokens = function (e, t) {
				return to.walkTokens(e, t);
			}),
			(tc.parseInline = to.parseInline),
			(tc.Parser = ts),
			(tc.parser = ts.parse),
			(tc.Renderer = ti),
			(tc.TextRenderer = tl),
